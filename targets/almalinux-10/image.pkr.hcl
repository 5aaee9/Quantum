source "qemu" "almalinux-10" {
  ...
  iso_url           = "https://raw.repo.almalinux.org/almalinux/10/isos/x86_64/AlmaLinux-10-latest-x86_64-boot.iso"

  output_directory  = "outputs/almalinux-10"
  accelerator       = "kvm"
  ...
  boot_command      = [
    "<up>e<down><down><end> inst.text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/kickstart-10.cfg<leftCtrlOn>x<leftCtrlOff>"
  ]
...
build {
  ...
  provisioner "shell" {
    scripts = [
      "./scripts/almalinux/00-update-dnf.sh",
