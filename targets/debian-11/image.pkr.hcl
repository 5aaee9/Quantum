source "qemu" "debian-11" {
  iso_checksum      = "file:https://cdimage.debian.org/cdimage/archive/11.11.0/amd64/iso-cd/SHA256SUMS"
  iso_url           = "https://cdimage.debian.org/cdimage/archive/11.11.0/amd64/iso-cd/debian-11.11.0-amd64-netinst.iso"

  output_directory  = "outputs/debian-11"
  accelerator       = "kvm"
  ...
  boot_command      = ["<down>e<down><down><down><end>net.ifnames=0 priority=critical auto=true preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/preseed-debian-11.cfg<leftCtrlOn>x<leftCtrlOff>"]
